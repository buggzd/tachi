...
void main(){
 ...
 // Reuse the preceding segment endpoint only when its floating coordinate is
 // exactly the current point. Otherwise retain reference sampling and arithmetic.
 float previousX=-2.,previousDepth=0.,previousShift=0.;
 ...
 for(int i=-33;i<=33;i++){
  ...
  float d,shift;
  if(q.x==previousX){d=previousDepth;shift=previousShift;}
  else{d=depthAt(q);shift=shiftAt(q);}
  float projected=q.x+shift,error=abs(projected-uv.x);
  ...
  float dr=depthAt(r),rs=shiftAt(r),end=r.x+rs,span=end-projected;
  previousX=r.x;previousDepth=dr;previousShift=rs;
