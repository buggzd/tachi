...
`mesh-trials.html` 新增默认“背景局部液化”，原始像素、连续弹性网格及其他策略继续保留。只对横向深度跃变的露出侧增加远景位移；沿背景距离以 smoothstep 衰减，羽化 8–256 个源图像素，拉伸强度 0–100%。不融合历史 RGB、不模糊颜色。背景深度差采用 0.025–0.12 连续置信度，沿搜索方向选择最强支持，不再首次命中即停止；8 次深度引导的二维位移扩散连接相邻行，跨越明显深度断层时禁止传播。近景按原深度视差参与遮挡竞争；未覆盖处仍用原 gather 填充。它不是无洞保证，也不能修复深度错时。绿色诊断表示额外拉伸，紫色表示仍需原始填充。

位移场在 CPU 按深度帧缓存，上传 RG16F 双眼纹理，GPU 按连续投影区间反求源坐标，减少拉伸后离散点之间的覆盖缺口；当前只是网页画质实验，没有手机计时。复杂层次、低分辨率边界与深度抖动可能导致错误选面、拉弯纹理和跳边。
...
连续性改进验证：新增缺失边缘行和旧门限附近扰动测试，共 20 项单元测试通过；四段片源的浏览器参数、播放/循环、SBS、零强度与 GL 检查通过。二维求解增加 CPU 工作，尚无手机性能数据；它不等同于可靠语义轮廓，也不保证跨帧稳定。30% 拉伸仍会有未覆盖区，紫色诊断不应被解释为全部都是断点。

### 严格同帧几何对照

默认 p4：392×224，每视频帧使用该帧实际推理结果，stride=1、delay=0；逐帧独立精确 P2/P98 归一化，没有范围 EMA 或像素历史融合。页面显示深度采样对应的视频帧号与错位帧数。它用于隔离几何/液化问题，不模拟手机推理延迟，也不保证模型轮廓准确或深度时间稳定。旧 p0–p3 保留可选，原盲测 manifest 不改。

运行 `python StereoLab/experiments/prepare_aligned_depth.py` 从已生成的两组数据集 float 推理缓存导出独立 `aligned.json` 和 p4 深度；不会重新推理或修改旧深度。高动态片段的 579 帧对应深度 579，其他片段同样逐帧配对。取消范围平滑会改变深度尺度响应，因此此档同时隔离延迟和历史，不是只改变延迟的单变量实验。
