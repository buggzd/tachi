...
1. Download the signed ARM64 APK from [GitHub Releases](https://github.com/buggzd/tachi/releases). GitHub Actions builds Lite; maintainers build and validate Full locally before attaching it to the release. Check available assets. The variants are Lite (native 2D / flat SBS) and Full (adds the 392×224 strictly paired liquid depth profile and QNN; currently validated on SM8850/V81). See the [build guide](docs/REALTIME_SBS.md). To build a debug APK yourself:
