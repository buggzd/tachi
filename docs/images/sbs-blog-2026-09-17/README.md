...
`06`–`12` 来自开发时 `StereoLab/mesh-trials.html` 的“高动态 · 30 秒”案例。按用户要求用于本篇技术说明，未包含完整视频、音轨、账号或服务器信息。“高动态”指运动，不是 HDR。素材版权归原权利人，这些截图不作为原创示意图授权。
...
新增 `10-motion-profiles-delayed.png`（p0/p1/p2）和 `11-motion-profiles-aligned.png`（p3/p4）。每行按深度、gather33、liquid 排列，RGB 均为第 579 帧，所有列裁切区域均为 `[535,370,1065,665]`，保留原像素大小。p0/p1/p2 的深度索引为 288，即采样自视频第 576 帧；p3/p4 为 579。p0/p2/p3 有像素历史融合，p1 去掉像素历史但保留范围 EMA；p4 两项均关闭。不能把这些组合档位整体解释成只改变分辨率的实验。
