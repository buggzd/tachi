...
## 第 74 帧深度、第 576 帧错帧对照、第 579 帧完整流程
...
`13-motion-history-control.png` 使用第576帧的p0/p1对照，仅像素历史开关不同；`14-motion-history-detail.png` 展示p2/p3/p4的组合档位。左列深度、右列液化左眼，统一裁切 `[535,370,1065,665]` 并最近邻放大两倍。p0–p2深度索引287，采样自574；p3/p4索引576。这些截图主要说明旧深度用于当前 RGB 的错帧问题，不能据此把断层归因于历史融合；p0/p1 都有两帧错位，p2/p3/p4 也不是仅改变配对时间的单变量实验。文件名沿用最初命名，图内标题与正文已纠正。`15-motion-source-74-576.png` 提供两帧的全幅原始画面。
