...
9 月 16 日 SBS 切换后 HyperOS 报告外屏 disabled，当时未完成 SBS 复测。
9 月 17 日补验时输出已恢复，原生状态为 stereo ready、applied，非 disabled。
已安装修复版的 SBS 首页 Hero → 内容行、内容行横向切换均产生 1 个通知、
背景改变且焦点标记为 1 个。随后覆盖安装 v0.4.0 Full Debug，SBS 自动恢复，
首页 Hero → 内容行再次通过，原生播放与实时深度能力入口均可用。
本次没有补齐 SBS 详情选集和完整播放/显示生命周期矩阵，不能扩大为全量验收。
