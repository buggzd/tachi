...
推理和两眼独立的深度偏移。当前完成的是源码与桌面验证，尚未完成应用实机验收。
...
模型不可用、图中存在 CPU fallback、取帧被 CORS 拒绝、原生回复超时或绘制失败，
...
4. 输出检查有限值，以 P5/P95 映射成 8 位相对深度；纯色/淡入淡出等没有可用
   深度范围的帧保持平面，继续处理后续帧，不锁定整个转换会话。更新两个独立
...
性能数据。深度只有双线性采样，尚未加入运动传播、时域滤波或更完整遮挡修复；
需要在实机检查运动边缘、补洞拉伸和逐帧归一化带来的闪动。

从捕获时刻起超过 150 ms 的深度不应用，已显示深度也在该期限后恢复平面内容；
两眼共享同一次有效性判断，避免过期边界产生左右不一致。每次生命周期重置使用
...
按用户要求，本轮不连接 ADB、不安装或操作设备。后续一次性执行：
...
5. **质量与恢复**：快速运动、场景切换、细线/头发/遮挡边缘；seek、隐藏/恢复、断连、
   模式切换、renderer 丢失、字幕保护和错误重试。150 ms 回退不能掩盖持续掉深度。
