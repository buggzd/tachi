...
- qinglong-vr-3d，up主青龙圣者的自研项目：[bilibili.com/video/BV1Svbf6sEvf ](https://www.bilibili.com/video/BV1Svbf6sEvf)，下面会单独介绍它的 Quality 渲染方法。
...
所以这张图主要说明的是**旧深度驱动了新 RGB 的错误位移**，不能把这些断层归因于历史融合。p1 已经关掉像素历史，问题仍然存在，正好能说明只关历史并不能解决错帧。
...
我们试过历史融合、范围 EMA、历史裁剪和不同门限，也试过用光流把旧深度往新位置搬。桌面合成测试里，带可信度检查的 DIS 光流传播让移动轮廓误差下降了约 63%，但新露出的区域还是难处理，而且要额外获取运动输入、判断遮挡，手机上的成本也没有验证完。[桌面实验](../performance/2026-09-11-desktop-optimization/README.md)
...
原生版把背景修正和八轮二维扩散都搬进了 GLES compute，左右眼一起计算。保留 392 深度、0.85 强度、96 px 羽化和 65% 拉伸，先确认移植没有改变已经看好的效果，再谈提速。桌面对照里，包括高动态第 579 帧在内，位移误差低于 0.00001 个像素。这只是同输入下的算法数值验证，手机的 QNN 量化深度和网页 float 深度仍有差别。[移植记录](../performance/2026-09-14-native-liquid/README.md)
...
先把优化的位置标在管线上。实线是当前帧的数据流，虚线表示槽位释放后的补采机会；图里的 CPU 交接也说明，现在还不是 GPU 到 NPU 的零拷贝。

```mermaid
flowchart TD
    A["视频解码 / RGB 帧 + PTS"] --> B["① 采样判断：按 PTS 选帧"]
    B --> C["② 获取空闲槽：最多两个"]
    C --> D["GPU 捕获：保存同帧 RGB，并生成模型小图"]
    D --> E["主机输入交接 → NPU 深度推理 → 输出交接"]
    E --> F["GPU 深度归一化 / 接受 RGB 与深度配对"]
    D --> RGB["保留 RGB 快照，等待自己的深度"]
    RGB --> F
    F --> G["③ 交换 RGB 纹理所有权 / 释放捕获槽"]
    G -. "符合条件时先补采" .-> C
    G --> H["④ GPU 液化：背景修正 + 八轮扩散"]
    H --> I["⑤ 双眼重建 / 更新 SBS 缓存"]
    I --> J["显示：没有新配对时复用缓存"]
    classDef changed fill:#e7f3ff,stroke:#397ab8,color:#17324d;
    class B,C,G,H,I changed;
```

①解决采样节拍误跳帧，②和③解决槽位周转，④减少扩散的重复读取，⑤避免重复合成同一对画面。它们处理的是不同位置的问题，下面顺着这些位置看。
...
把上图④的**一轮扩散**放大，就是下面这个变化。八轮仍然依次执行，没有减少轮数，也没有把所有轮次同时算。

```mermaid
flowchart LR
    subgraph OLD["原来：各读各的"]
        A["8×8 个线程"] --> B["每个线程从纹理读取 3×3 邻域"]
        B --> C["各自计算本轮结果"]
    end
    subgraph NEW["采用：合作读取"]
        D["8×8 个线程"] --> E["合作加载 10×10 区域
含一圈边界"]
        E --> F["工作组同步"]
        F --> G["从共享内存读取各自的 3×3 邻域"]
        G --> H["各自计算本轮结果"]
    end
```
...
对应管线①到③，可以把“补采”理解成下面这个分支。它重试的是还留在原纹理中的那一帧，不是把所有漏掉的帧排队存起来。

```mermaid
flowchart TD
    A["当前视频帧到达"] --> B{"按 PTS 判断需要采样？"}
    B -- 否 --> X["本次不采样"]
    B -- 是 --> C{"读回与捕获槽可用？"}
    C -- 是 --> D["捕获 RGB + 模型输入
成功后推进采样节拍"]
    C -- 否 --> E["记住当前帧的补采机会"]
    E --> F["后续绘制 / GPU 完成交接时检查"]
    F --> G{"仍是同一帧、无新帧等待
且时间轴与采样状态有效？"}
    G -- 否 --> H["放弃旧机会"]
    G -- 是 --> C
...
图中的回路靠后续事件触发，不会原地忙等。新配对没回来时，眼镜仍可显示已有缓存；取消的是一次多余的主动绘制请求。
...
这是手机上的独立测试入口，每眼仍按 1920×1080 合成，取同一媒体区间做顺序短测。它没有完整 Jellyfin 播放、字幕和外接眼镜的负载，不能拿这个 16.34 直接和后面的完整 tachi 数据算提升比例。温度和频率也没有固定，这张表记录的是当轮观察到的改善，不是长期帧率保证。[采样与共享邻域优化记录](../performance/2026-09-15-liquid-24hz/README.md)
...
如果只看前两轮，会觉得有 2.6% 的提升；回切后发现，原版自己也变快了。独立线程相对回切只剩约 1.7% 的优势，加上温度和启动状态不同，还不足以确认稳定收益，所以没有默认开启。[轮询对照记录](../performance/2026-09-15-gpu-poll-short/README.md)
...
这次调整的就是管线③和④的交界。用 A 表示刚配对完成的画面，B 表示下一张符合补采条件的画面，看提交顺序更直观：

```mermaid
flowchart LR
    subgraph OLD["原顺序"]
        O1["A 配对就绪"] --> O2["提交 A 液化"]
        O2 --> O3["释放槽 / 尝试捕获 B"]
        O3 --> O4["B 的输入完成信号
可能排在 A 液化后"]
    end
    subgraph V1["第一版：等待搬到了后面"]
        P1["A 配对就绪"] --> P2["释放槽 / 尝试捕获 B"]
        P2 --> P3["等下一次绘制回调"]
        P3 --> P4["才提交 A 液化"]
    end
    subgraph V2["保留版：补采后立即液化"]
        N1["A 配对就绪"] --> N2["释放槽 / 尝试捕获 B"]
        N2 --> N3["立即提交 A 液化"]
        N3 --> N4["B 输入更早入队
A 不额外等绘制回调"]
    end
    classDef wait fill:#fff0df,stroke:#c58534,color:#573811;
    class O4,P3 wait;
```
...
这轮约增加了 4.1% 的新配对吞吐，软件延迟指标也改善了，所以保留了这个顺序。不过“绘制开始”还没有包含 GPU 绘制完成和屏幕发光；阶段耗时取自滚动均值的采样统计，也不能当成逐帧端到端延迟。[排队优化记录](../performance/2026-09-15-capture-order/README.md)
...
所以这两项都保留为实验开关，默认关闭。和前面的邻域共享不同，它们还没有足够的整机收益支持。这几轮也是顺序短测，不能反过来断言候选在所有情况下都更慢。[shader 对照记录](../performance/2026-09-16-liquid-shaders/README.md)
