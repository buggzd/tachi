...
这里用第 74 帧来看深度。这一帧有近处的人物和膝盖、身后的座椅、舱壁，层次比第 579 帧明显。第 576 帧则留着看历史融合的问题，最后再用 579 走完整条管线。
...
### 第 576 帧：历史融合会把人物弄出断层

另一种很自然的想法是：深度图抖，那就把前后两次深度混合一下。

静止画面这样做是有用的，但物体移动时，同一个屏幕坐标上可能已经换了东西。上一帧这里是人，这一帧这里是天空，直接融合就会把旧深度留在新画面里。问题也不只是轮廓拖尾：同一个人物内部混进了不同时间的深度，重建时不同区域会被移到不同位置，看起来就像接不上了。

![第576帧p0与p1的历史融合对照](../images/sbs-blog-2026-09-17/13-motion-history-control.png)

*先看控制变量最接近的一组：p0 有像素历史，p1 关闭像素历史，两者都保留范围 EMA，尺寸都是 266×154，深度都采样自第 574 帧。左边深度、右边液化左眼，相同区域最近邻放大两倍。*

这组深度里，p0 的人物内部能看到斑块，p1 相对均匀。不过右边输出的差异没有灰度图那么醒目，不能说关掉历史就把所有边缘问题都修好了。

![第576帧p2、p3、p4的深度断层与重建结果](../images/sbs-blog-2026-09-17/14-motion-history-detail.png)

*p2 是 392 的旧历史档位；p3 已经同帧，但仍有历史融合；p4 是无历史的 392 同帧档位。三行的裁切和液化参数一致。它们有多个设置不同，用于观察问题，不能当成只切换历史开关的实验。*

第 576 帧主要看这里的断层：对着人物的手臂、躯干和弯曲的小腿看，深度里的不连续会变成输出里的轮廓错接。尤其是 p2 这一行，人物内部的深度并不均匀，右侧局部也出现了明显的切口。p4 仍有细碎边缘，它也不是正确答案的标准图。

这里要把两个问题分开。**深度与 RGB 错帧**，是拿旧画面的深度去处理新画面；**历史融合**，是当前交出来的深度本身还混着旧状态。p3 已经按同一帧配对，但历史还在，所以仅仅保证帧号一致，并不会自动消除融合带来的问题。反过来，p0–p2 还有两帧的配对延迟，它们的全部瑕疵也不能都归到历史融合上。
...
折腾之后才发现，画面变化得慢不一定就对了，有时候只是旧状态消失得更慢。静态截图可以看断层的位置，融合产生和消退的过程还得回测试页连续播放。[桌面实验](../performance/2026-09-11-desktop-optimization/README.md)
