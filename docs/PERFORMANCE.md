...
正在验证的实现见[24 Hz 采样诊断与液化邻域复用](performance/2026-09-15-liquid-24hz/README.md)：共享 tile 保持原公式，液化采样按已知媒体 PTS 选帧；同素材手机短测 16.34→19.96→21.21 Hz，后续同帧补采和取消无新结果重绘达到 23.48 Hz，尚未保证 24 Hz，不能代替外接显示长测。

完整 tachi 实际 Jellyfin HEVC/VTT 外接 SBS 已完成[连续 21 分 25 秒](performance/2026-09-15-native-liquid-product/README.md)，平均 22.54 Hz。报告分开记录新配对帧、GL 跨帧、补采、滚动阶段耗时和呈现采样缺口；充电温度不能外推续航，混合缓存温度数据不作为当前传感器值。

当前 liquid 实验包采用[补采后立即提交液化](performance/2026-09-15-capture-order/README.md)，同片源短测 22.68→23.62 Hz，捕获到绘制开始 85.80→82.06 ms。该指标不含 GPU 绘制完成或物理呈现；先前仅把液化推迟到绘制的方案转移了等待，未采用。

两轮扩散融合与最终重建终点复用已完成[实际 GLES 与完整 tachi 对照](performance/2026-09-16-liquid-shaders/README.md)。八轮扩散、FP32 和搜索规则不变；数值等价通过，但未显示明确端到端收益，两个候选暂不默认启用。后续先分析最终搜索的算术、纹理依赖和寄存器占用，再决定是否扩大改写范围。
